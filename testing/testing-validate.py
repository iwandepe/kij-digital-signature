...
def main():
	...
	result = True
	base_path = 'C:/developing/project/project-python/kij/kij-digital-signature/'
	input_path = base_path + 'static/testing-validate/'
	output_path = base_path + 'static/testing-validate/out/'
	...
	try:
		...
		doc.Save(output_path + 'file_withApprovalField_output.pdf', SDFDoc.e_remove_unused)
		copyfile(output_path + 'file_withApprovalField_output.pdf', input_path + 'file_withApprovalField.pdf')
	except Exception as e:
		print(e.args)
		result = False
	#################### TEST 1: certify a PDF.
	try:
		CertifyPDF(input_path + 'file_withApprovalField.pdf',
			'PDFTronCertificationSig',
			input_path + 'container.pfx',
			'password',
			input_path + 'signature.png',
			output_path + 'file_withApprovalField_certified_output.pdf')
		PrintSignaturesInfo(output_path + 'file_withApprovalField_certified_output.pdf')
		copyfile(output_path + 'file_withApprovalField_certified_output.pdf', input_path + 'file_withApprovalField_certified.pdf')
	except Exception as e:
		print(e.args)
		result = False
	#################### TEST 2: approval-sign an existing, unsigned signature field in a PDF that already has a certified signature field.
	try:
		SignPDF(input_path + 'file_withApprovalField_certified.pdf',
			'PDFTronApprovalSig',
			input_path + 'container.pfx',
			'password',
			input_path + 'signature.png',
			output_path + 'file_withApprovalField_certified_approved_output.pdf')
		PrintSignaturesInfo(output_path + 'file_withApprovalField_certified_approved_output.pdf')
		copyfile(output_path + 'file_withApprovalField_certified_approved_output.pdf', input_path + 'file_withApprovalField_certified_approved.pdf')
	except Exception as e:
		print(e.args)
		result = False
	#################### TEST 3: Clear a certification from a document that is certified and has an approval signature.
	try:
		ClearSignature(input_path + 'file_withApprovalField_certified_approved.pdf',
			'PDFTronCertificationSig',
			output_path + 'file_withApprovalField_certified_approved_certcleared_output.pdf')
		PrintSignaturesInfo(output_path + 'file_withApprovalField_certified_approved_certcleared_output.pdf')
		copyfile(output_path + 'file_withApprovalField_certified_approved_certcleared_output.pdf', input_path + 'file_withApprovalField_certified_approved_certcleared.pdf')
	except Exception as e:
		print(e.args)
		result = False
	...
	try:
		if not VerifyAllAndPrint(input_path + "file_withApprovalField_certified_approved.pdf", input_path + "certificate.cer"):
			result = False
	except Exception as e:
		print(e.args)
		result = False
	...
	try:
		if not VerifySimple(input_path + 'file_withApprovalField_certified_approved.pdf', input_path + 'certificate.cer'):
			result = False
	except Exception as e:
		print(e.args)
		result = False
	...
	#try:
	#	if not TimestampAndEnableLTV(input_path + 'file.pdf',
	#		input_path + 'GlobalSignRootForTST.cer',
	#		input_path + 'signature.jpg',
	#		output_path+ 'file_DocTimeStamp_LTV.pdf'):
	#		result = False
	#except Exception as e:
	#	print(e.args)
	#	result = False
